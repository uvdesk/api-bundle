...
class Tickets extends AbstractController
{
     ...
     */
    public function fetchTickets(Request $request, ContainerInterface $container, UVDeskService $uvdesk, EntityManagerInterface $entityManager)
    {
        $json = [];

        $ticketRepository = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class);
        $userRepository = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class);

        if ($request->query->get('actAsType')) {
            switch ($request->query->get('actAsType')) {
                case 'customer':
                    $email = $request->query->get('actAsEmail');
                    $customer = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneByEmail($email);
                ...
                case 'agent':
                    $email = $request->query->get('actAsEmail');
                    $user = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneByEmail($email);
     ...
     * @return void
     */
    public function trashTicket(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $ticketId = $request->attributes->get('ticketId');
        $ticket = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class)->find($ticketId);
     ...
     * @return void
     */
    public function createTicket(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        ...
        if (
            ...
            $json['error'] = $container->get('translator')->trans('required fields: name, from, subject, message, actAsType or actAsEmail');
        ...
        if ($data) {
            ...
            if (! $error) {
                ...
                if (
                    ...
                    $user = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneByEmail(trim($data['actAsEmail']));
                ...
                $customer = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneByEmail($data['from']);

                if (
                    empty($customer)
                    || null == $customer->getCustomerInstance()
                ) {
                    $role = $entityManager->getRepository(CoreFrameworkBundleEntity\SupportRole::class)->findOneByCode('ROLE_CUSTOMER');
                ...
                if (array_key_exists('type', $data)) {
                    $ticketType = $entityManager->getRepository(CoreFrameworkBundleEntity\TicketType::class)->findOneByCode($data['type']);
     ...
     * @return void
     */
    public function viewTicket($ticketId, ContainerInterface $container, UVDeskService $uvdesk, EntityManagerInterface $entityManager)
    {
        $userRepository = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class);
        $ticketRepository = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class);
        ...
        }, $entityManager->getRepository(CoreFrameworkBundleEntity\TicketStatus::class)->findAll());
        ...
        }, $entityManager->getRepository(CoreFrameworkBundleEntity\TicketType::class)->findByIsActive(true));
        ...
        }, $entityManager->getRepository(CoreFrameworkBundleEntity\TicketPriority::class)->findAll());
     ...
     * @return void
     */
    public function deleteTicketForever(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $ticketId = $request->attributes->get('ticketId');
        $ticket = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class)->find($ticketId);
     ...
     * @return void
     */
    public function assignAgent(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $json = [];
        $data = $request->request->all() ?: json_decode($request->getContent(), true);
        $ticketId = $request->attributes->get('ticketId');
        $ticket = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class)->findOneBy(array('id' => $ticketId));

        if ($ticket) {
            if (isset($data['id'])) {
                $agent = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->find($data['id']);
            } else {
    ...
    public function addRemoveTicketCollaborator(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $json = [];
        $statusCode = Response::HTTP_OK;
        $content = $request->request->all() ?: json_decode($request->getContent(), true);

        $ticket = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class)->find($request->attributes->get('ticketId'));
        ...
        if (
            ...
            if ($content['email'] == $ticket->getCustomer()->getEmail()) {
                ...
                $supportRole = $entityManager->getRepository(CoreFrameworkBundleEntity\SupportRole::class)->findOneByCode('ROLE_CUSTOMER');
                $collaborator = $container->get('user.service')->createUserInstance($data['from'], $data['firstName'], $supportRole, $extras = ["active" => true]);
                $checkTicket = $entityManager->getRepository(CoreFrameworkBundleEntity\Ticket::class)->isTicketCollaborator($ticket, $content['email']);
                ...
                if (! $checkTicket) {
                    ...
                    $json['collaborator'] = $collaborator->getCustomerInstance() ? $collaborator->getCustomerInstance()->getPartialDetails() : $collaborator->getAgentInstance()->getPartialDetails();
            ...
            $collaborator = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneBy(array('id' => $content['id']));
     ...
     * @return void
     */
    public function downloadAttachment(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $attachmentId = $request->attributes->get('attachmentId');
        $attachmentRepository = $entityManager->getRepository(CoreFrameworkBundleEntity\Attachment::class);
     ...
     * @return void
     */
    public function downloadZipAttachment(Request $request, EntityManagerInterface $entityManager)
    {
     ...
     * @return void
     */
    public function editTicketProperties(Request $request, ContainerInterface $container, EntityManagerInterface $entityManager)
    {
        $json = [];
        $statusCode = Response::HTTP_OK;
        ...
        switch ($requestContent['property']) {
            case 'agent':
                $agent = $entityManager->getRepository(CoreFrameworkBundleEntity\User::class)->findOneById($requestContent['value']);
                if (empty($agent)) {
                    // User does not exist
                ...
                break;
            case 'status':
                $ticketStatus = $entityManager->getRepository(CoreFrameworkBundleEntity\TicketStatus::class)->findOneById((int) $requestContent['value']);
                ...
                break;
            case 'priority':
                $ticketPriority = $entityManager->getRepository(CoreFrameworkBundleEntity\TicketPriority::class)->findOneById($requestContent['value']);
                ...
                break;
            case 'group':
                $supportGroup = $entityManager->getRepository(CoreFrameworkBundleEntity\SupportGroup::class)->findOneById($requestContent['value']);
                ...
                break;
            case 'team':
                $supportTeam = $entityManager->getRepository(CoreFrameworkBundleEntity\SupportTeam::class)->findOneById($requestContent['value']);
                ...
                break;
            case 'type':
                $ticketType = $entityManager->getRepository(CoreFrameworkBundleEntity\TicketType::class)->findOneById($requestContent['value']);
                ...
                break;
            case 'label':
                $label = $entityManager->getRepository(CoreFrameworkBundleEntity\SupportLabel::class)->find($requestContent['value']);
